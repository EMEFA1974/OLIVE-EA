...
| 5 | Signal grade. Every base signal is graded against the filters in 1–3: **A** = passes all enabled filters, **B** = fails one, **C** = fails two or more. Only grades at or better than `InpMinGrade` become signals (zone, alert, EA trade). The indicator draws the lower grades as a small grey letter; hover over it to see which filters failed. | `InpMinGrade` (default A), `InpShowFiltered`, `InpFiltColor` | `InpMinGrade = C` with every filter off gives the v1.82 signals |
